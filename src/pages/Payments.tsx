...
export default function PaymentsPage() {
  ...
  // Custom Search Select State
  const [userSearch, setUserSearch] = useState("");
  const [showUserDropdown, setShowUserDropdown] = useState(false);
  const [paymentType, setPaymentType] = useState("general");

  const [formData, setFormData] = useState({
    user_id: "", amount: "", payment_method: "bKash", transaction_id: "", notes: ""
  });

  const filteredDropdownUsers = users.filter(u => 
    u.full_name?.toLowerCase().includes(userSearch.toLowerCase()) || 
    u.sid?.toLowerCase().includes(userSearch.toLowerCase()) ||
  ...
  async function handleSubmit(e: any) {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ user_id: "", amount: "", payment_method: "bKash", transaction_id: "", notes: "" });
      setUserSearch("");
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[calc(100vh-140px)]">
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="h-full flex items-center justify-center text-slate-500">No payments found.</div>
          ) : (
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 font-bold tracking-wider sticky top-0 shadow-sm z-10">
                <tr>
                  ...
                  <th className="px-6 py-4">Method & TXN</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredPayments.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 rounded bg-emerald-50 text-emerald-600 text-[10px] font-bold">COMPLETED</span>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2 relative">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="text"
                      placeholder="Search by Name, SID, Mobile..."
                      value={userSearch}
                      onChange={e => {
                      ...
                      className="w-full border border-slate-300 rounded-lg pl-9 pr-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                    />
                  </div>
                  {showUserDropdown && (
                      ...
                      {filteredDropdownUsers.length === 0 ? (
                        ...
                        filteredDropdownUsers.map(u => (
                          ...
                          >
                            ...
                            <div className="text-xs text-slate-500">SID: {u.sid} | Mob: {u.mobile} | Due: ${u.total_due || 0}</div>
                          </div>
                        ))
                      )}
                    </div>
                ...
                <div className="col-span-2">
                  ...
                  <select required value={paymentType} onChange={e => setPaymentType(e.target.value)} className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                    <option value="general">Package Due / General</option>
                    <option value="obb_fee">OBB Fee (Extends OBB Date)</option>
                  </select>
                </div>
                <div>
                ...
                <div>
                  ...
                  <select required value={formData.payment_method} onChange={e => setFormData({...formData, payment_method: e.target.value})} className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Transaction ID (Optional)</label>
                  <input type="text" value={formData.transaction_id} onChange={e => setFormData({...formData, transaction_id: e.target.value})} className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Notes</label>
                  <textarea rows={2} value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none"></textarea>
                </div>
              </div>
